...
def get_data(data, tag = False, meta = False):
    """
    ...
    """
    id_list, spelling = IdHelper.create_id_list(data, tag)
    ...
    if spelling:
        ...
        if meta:
            ...
            for i in range(length):#tqdm(range(length), total=length, desc=f"[#] "):
                for id, url, format, name, created, last_modified, description in FetchHelper.fetch_dataset_meta(id_list[i]):
...
#get_data(["staedtische_einrichtunge"], meta = True)
